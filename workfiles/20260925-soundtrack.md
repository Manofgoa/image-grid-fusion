...
| Mix | Played **over** the grid's own video sound, never replacing it |
...
| Per-video sound | Each video's own sound can be turned off individually |
| Persistence | Not persisted, like every effect |

### Mixing

- **Preview**: a second `MediaPlayer`, playing the soundtrack in step with the grid's position
  (looped, same drift correction as `PreviewSound`), next to the existing one.
- **Exports**: `VideoEncoder` gains a mixing step — both sources decoded to PCM at a common format,
  summed sample by sample with the soundtrack's gain, clamped to the 16-bit range, then encoded to
  AAC as today. With no video sound, the soundtrack alone is written; with no soundtrack, today's
  path is unchanged.
...
- [ ] Which video sound does the soundtrack mix over — the single elected sound of today
      (`SoundSource`), or **every** video with sound, mixed together (a much larger change)?
- [ ] Per-video mute: left to `workfiles/20260925-video-mute.md`, or absorbed into this workfile?
- [ ] Where does the soundtrack's toggle live — a **Global effects row** created here (the one
      `global-fade.md` plans), a button of the effects toolbar enabled without a selected cell, or
      the top bar?
- [ ] How is the file picked — a *Browse* button in the options, and/or a file dropped on the row?
...
| 5 | Which video sound does the soundtrack mix over? | | 2026-09-25 |
| 6 | Per-video mute: here or in the video-mute workfile? | | 2026-09-25 |
| 7 | Where does the soundtrack's toggle live? | | 2026-09-25 |
| 8 | How is the file picked? | | 2026-09-25 |
