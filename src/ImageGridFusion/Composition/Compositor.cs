...
/// <summary>What a cell shows: a bitmap, the color of its bands, and the actions on the image, if any.</summary>
public readonly record struct Frame(Bitmap Bitmap, Color Dominant, ImageLook? Look = null)
{
...
public static class Compositor
{
    ...
    public static Bitmap Render(IReadOnlyList<SourceImage> images, GridLayout layout, double threshold = FitCalculator.DefaultCropThreshold) =>
        Render(images.Select(i => new Frame(i.Bitmap, i.Dominant, i.Look)).ToList(), layout, threshold);
    ...
    public static void Draw(Graphics g, IReadOnlyList<SourceImage> images, GridLayout layout, Size canvas, double threshold = FitCalculator.DefaultCropThreshold) =>
        Draw(g, images.Select(i => new Frame(i.Bitmap, i.Dominant, i.Look)).ToList(), layout, canvas, threshold);
    ...
    public static void DrawCell(Graphics g, Frame frame, Rectangle cell, double threshold = FitCalculator.DefaultCropThreshold)
    {
        ...
        using (var brush = new SolidBrush(look.Grayscale ? Gray(frame.Dominant) : frame.Dominant))
        {
        ...
        if (look is { Rotation: 0, FlipX: false, FlipY: false })
        {
            g.DrawImage(
                frame.Bitmap,
                destination,
        ...
        {
            DrawOriented(g, frame.Bitmap, look, fit.Source, destination, attributes);
        }
    ...
    /// <summary>
    /// Draws the part <paramref name="source"/> of the oriented image into <paramref name="destination"/>:
    /// the matching bitmap rectangle goes to a parallelogram whose corners follow the rotation and flips.
    /// </summary>
    private static void DrawOriented(Graphics g, Bitmap bitmap, ImageLook look, RectangleF source, Rectangle destination, ImageAttributes attributes)
    {
        using var orientation = look.Orientation(bitmap.Size);
        using var inverse = orientation.Clone();
        inverse.Invert();
        ...
        var bitmapPart = RectangleF.FromLTRB(corners.Min(c => c.X), corners.Min(c => c.Y), corners.Max(c => c.X), corners.Max(c => c.Y));
